...
def orchestrate_generation(user_story, bdd_scenario, tech_stack, figma_file_key=None, figma_access_token=None):
    ...
    # model_code = generate_model(backend_templates['model'], entity_name, action_name)
    # repository_code= generate_repository(backend_templates['repository'], entity_name, entity_name.lower(), action_name)
    # routes_code = generate_routes(backend_templates['routes'], entity_name, entity_name.lower())
    # service_code = generate_service(backend_templates['service'], entity_name, action_name)
    controller_code = generate_controller(backend_templates,
                                          entity_name, entity_name.lower(), action_name)
